...
const checkErrors = () => {
  if (hasNameError || hasSurnameError || hasEmailError || hasPhoneError || hasMessageError || hasCheckError) {
...
const onSubmit = (e) => {
  ...
  if (isNameValid && isSurnameValid && isEmailValid && isPhoneValid && isMessageValid && isChecked) {
    $form.trigger("submit");
  } else {
    $submit.attr("disabled", true);
  }
